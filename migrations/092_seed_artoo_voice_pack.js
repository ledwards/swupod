/**
 * Migration 092: publish R2-D2 as a Protect the Pod voice pack.
 *
 * Unlike every other pack here, nothing in this one is synthesised or spoken.
 * Astromech is not a language and R2 is not saying words, so there is no line for a
 * voice to read — the meaning is entirely in the tone, and tone has to be recorded.
 * The eleven cues are real recordings, assigned to slots by measured character and
 ...
 *
 * The clips are built by `npm run voice:generate -- --all-clips --pack artoo`.
 *
 * IDEMPOTENCY
 * ===========
...
const ASSET_DIR = join(dirname(fileURLToPath(import.meta.url)), 'assets', 'artoo')
...
export async function run(client) {
  ...
  if (!existsSync(logoPath)) {
    throw new Error(`092: missing ${logoPath} — the redeem page leads with this logo.`)
  }
  const logo = readFileSync(logoPath)
  if (logo.length === 0) throw new Error(`092: ${logoPath} is empty`)

  const clips = CLIPS_AT_092.map((clipType) => {
    const file = join(ASSET_DIR, `${clipType}.mp3`)
    if (!existsSync(file)) throw new Error(`092: missing ${file}`)
    const audio = readFileSync(file)
    if (audio.length === 0) throw new Error(`092: ${file} is empty`)
  ...
  if (rows.length === 0) throw new Error('092: pack row missing immediately after insert')
  ...
  console.log(`092: R2-D2 published (${PACK.code}) — ${written} of ${clips.length} clip(s) written.`)
}
