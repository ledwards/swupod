/**
 * Booster-pool generation tracking — pure, shared by the format API routes (live
 * pools) and the backfill migrations (pools created before tracking existed), so the
 * two cannot drift apart.
 *
 * Covers every format that opens ordinary boosters and stores them on `card_pools`:
 * Chaos Sealed, Pack Wars and Pack Blitz.
 *
 * BACKGROUND
 * ==========
 ...
 * `/api/pools` and Draft writes it at draft start, but the format routes inserted
 * their pool and returned, so nothing they opened was ever attributed to the puller
 * — a Showcase leader pulled in Chaos Sealed simply never appeared in their
 * collection.
 *
 * SPEC
 * ====
 ...
 *   cards into booster distribution stats. Only Chaos Sealed can contain one; the
 *   check is harmless for the formats that cannot.
 */
import { PACK_SLOT_TYPES } from '../utils/packSlotTypes'
import { PROMO_SET_CODES } from './chaosSealedSelection'

interface BoosterPool {
...
export interface BoosterPoolTrackingRecord {
  card: StoredCard
  options: {
    packType: 'booster'
    sourceType: 'sealed'
...
/**
 * Build the `card_generations` records for a booster pool.
 *
 ...
 */
export function buildBoosterPoolTrackingRecords(
  packs: StoredPack[] | null | undefined,
  pool: BoosterPool
): BoosterPoolTrackingRecord[] {
  if (!Array.isArray(packs)) return []

  const records: BoosterPoolTrackingRecord[] = []
