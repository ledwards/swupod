...
async function runTests(): Promise<void> {
  ...
  test('FIXED: Set 7+ paired copies ride close on the line, never in the same pack', () => {
    ...
    // Real box 001 line order: intra-boot pair gaps <=4 packs ≈ 62% (histogram {1:32,3:17,4:1});
    // measured belt mean 0.688. Aggregated over ~40 boots (~1900 gaps) the floor 0.55 sits
    // ~13σ below the true mean (flake-free) yet ~24σ above a scrambled-pairing bug (~0.30),
    // preserving the bug-detection intent while eliminating the single-boot variance flake.
    assert(closeRatio >= 0.55,
      `SPEC: >=55% of pair gaps within 4 packs (real box ≈62%, belt ≈69%) aggregated over ${pairedBoots} boots, got ${(closeRatio * 100).toFixed(0)}%`)
    assert(minGap >= 2,
