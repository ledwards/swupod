...
function imageBackground(url: string): CSSProperties {
  return {
    backgroundImage: `url("${url.replace(/["\\]/g, '\\$&')}")`,
  }
}

function allTabBackground(): CSSProperties {
  return {
    ...imageBackground(RULE_WITH_RESPECT_HYPERSPACE_IMAGE),
    ...
    <div className="draft-slideshow-tabs" aria-label="Players">
      ...
      <div className="draft-slideshow-tab-list" role="list">
        {seats.map(seat => {
          ...
          const tabStyle = imageBackground(seat.avatarUrl || AVATAR_FALLBACK)
