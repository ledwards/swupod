#!/usr/bin/env bash
# Regenerate src/data/duplicateStats.json (powers the /duplicate-rates page).
#
...
# optional — if no DB is reachable the page just shows Simulated + Theoretical.
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "✓ Done. Rebuild/redeploy to publish the refreshed /duplicate-rates page."
