...
export default function PodPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    const isSoloDraft = podData.draft?.settings?.isSolo === true
    ...
    trackEvent(LimitedAnalyticsEvents.LIMITED_PLAY_PAGE_VIEWED, {
      ...buildLimitedContext({
        ...
        podShareId: shareId,
        routeTemplate: '/draft/[shareId]/pod',
      }),
  ...
  const getDeckData = () => {
    ...
    const poolName = state.poolName || myPool.name || formatPoolLabel(myPool.setCode, 'draft')
  ...
  const exportOwnDeckImage = async () => {
    ...
    try {
      ...
      const displayName = state.poolName || myPool.name || formatPoolLabel(myPool.setCode, 'draft')
  ...
  const viewPlayerDeck = async (playerId: string) => {
    ...
    try {
      ...
      const displayName = state.poolName || poolData.name || formatPoolLabel(poolData.setCode, 'draft')
      ctx.fillText(displayName, width / 2, currentY)
      currentY += titleHeight
  ...
  return (
    <div className="page-with-chat">
      ...
      {!isSolo && <ChatPanel shareId={shareId} analyticsContext={getLimitedAnalyticsContext()} />}
    </div>
  )
}
