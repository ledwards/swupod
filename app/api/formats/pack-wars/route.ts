// @ts-nocheck
// POST /api/formats/pack-wars - Generate a Pack Wars pool (2 packs)
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    trackBulkGenerations(
      buildBoosterPoolTrackingRecords([pack1, pack2], { poolId: pool.id, shareId, userId })
    ).catch(err => {
      console.error('Failed to track pack wars generations:', err)
    })

    const APP_URL = process.env.APP_URL || process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
    const shareUrl = `${APP_URL}/formats/pack-wars/${shareId}`
