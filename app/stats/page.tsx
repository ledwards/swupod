...
import { PICK_PREFERENCE_STATS } from '@/src/data/pickPreferences'
import type { PickPreferenceCardStat } from '@/src/services/pickPreferenceGrades'
...
type SortKey = 'cardName' | 'rarity' | 'avgPickPosition' | 'firstPickPct' | 'timesPicked' | 'pickPref'

// Pick-preference aggregates (within-aspect pick rates from reconstructed
// human draft contests) — generated by scripts/analyze-pick-preferences.ts.
// Aggregate card stats only; sets without a data file simply omit the column.
// The registry also backs the pick-preference tier grades in card-data.
const PICK_PREFERENCES = PICK_PREFERENCE_STATS
...
function DraftTab({ setCode, includeBots, includeHumans, startDate, endDate, user, showYou, showAll, showTop, showTournament, legendProps, isBlurred, canSeeFullStats }: TabProps) {
  ...
  // Pick-preference lookup for this set (name|subtitle -> {aRate, aSeen, rating})
  const pickPrefData = PICK_PREFERENCES[setCode]
  const pickPrefMap = useMemo(() => {
    const map = new Map<string, PickPreferenceCardStat>()
    for (const c of pickPrefData?.cards || []) map.set(`${c.name}|${c.subtitle || ''}`, c)
    return map
  }, [pickPrefData])
  const pickPrefFor = (card: { cardName: string, subtitle?: string | null }) =>
    pickPrefMap.get(`${card.cardName}|${card.subtitle || ''}`) || pickPrefMap.get(`${card.cardName}|`)
  ...
  const sortedCards = useMemo(() => {
    ...
    return [...cardData.cards].sort((a, b) => {
      ...
      switch (cardSortKey) {
        ...
        case 'pickPref': cmp = (pickPrefFor(a)?.aRate ?? -1) - (pickPrefFor(b)?.aRate ?? -1); break
      }
      return cardSortAsc ? cmp : -cmp
    })
  }, [cardData?.cards, cardSortKey, cardSortAsc, pickPrefMap])
            ...
            <table className="stats-table">
              <thead>
                <tr>
                  ...
                  {pickPrefData && <CardSortHeader label="Pick Pref" col="pickPref" title="How often players took this card when choosing among cards of the same aspect in the pack — controls for color commitment. From reconstructed human picks" />}
                ...
                {filteredCards.map(card => {
                  ...
                  return (
                    <tr key={card.cardId}>
                      ...
                      />
                      {pickPrefData && (() => {
                        const pp = pickPrefFor(card)
                        return <td>{pp?.aRate != null ? `${(pp.aRate * 100).toFixed(0)}%` : '—'}</td>
                      })()}
                      <StatsCell
                        {...cellProps}
                        you={youCard?.timesPicked}
