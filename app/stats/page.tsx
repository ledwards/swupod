...
import pickPreferencesASH from '@/src/data/pickPreferences/ASH.json'
...
const PICK_PREFERENCES: Record<string, { minSeen: number, cards: { name: string, subtitle: string, aRate: number, aSeen: number, rating: number }[] }> = {
  ASH: pickPreferencesASH,
}
...
function DraftTab({ setCode, includeBots, includeHumans, startDate, endDate, user, showYou, showAll, showTop, showTournament, legendProps, isBlurred, canSeeFullStats }: TabProps) {
  ...
  const pickPrefMap = useMemo(() => {
    const map = new Map<string, { aRate: number, aSeen: number, rating: number }>()
                  ...
                  return (
                    <tr key={card.cardId}>
                      ...
                      {pickPrefData && (() => {
                        ...
                        return <td>{pp ? `${(pp.aRate * 100).toFixed(0)}%` : '—'}</td>
                      })()}
                      <StatsCell
                        {...cellProps}
                        you={youCard?.timesPicked}
